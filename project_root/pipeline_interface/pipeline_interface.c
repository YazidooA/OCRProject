...
#define ACCEPT_P1_THR  0.90f   // if top1 prob >= this, we accept directly (if margin OK)
...
/* -------------------- Small local structs -------------------- */
typedef struct { int x,y,w,h; } Box;  // simple integer rectangle
...
static int save_buf784_bmp(const Uint8 *buf784, const char *path) {
    if (!buf784 || !path) return -1;  // invalid parameters

    SDL_Surface *surf = SDL_CreateRGBSurfaceWithFormat(
        0, 28, 28, 32, SDL_PIXELFORMAT_ARGB8888);  // 28x28 ARGB surface
    if (!surf) return -2;
    if (SDL_LockSurface(surf) != 0) { SDL_FreeSurface(surf); return -3; }
    ...
    if (SDL_SaveBMP(surf, path) != 0) { SDL_FreeSurface(surf); return -4; }
...
static int otsu_from_hist(const int hist[256], int total) {
    if (total <= 0) return 128;                            // degenerate case
    double sum = 0.0;
    for (int t = 0; t < 256; ++t) sum += (double)t * hist[t];  // sum of intensities

    double sumB = 0.0, maxVar = -1.0;
    int wB = 0, bestT = 128;
    for (int t = 0; t < 256; ++t) {
        wB += hist[t]; if (wB == 0) continue;              // background weight
        int wF = total - wB;  if (wF == 0) break;          // foreground weight
...
static void binarize_roi(SDL_Surface *s, SDL_Rect roi, Uint8 *bin) {
    ...
    if (!G) { SDL_UnlockSurface(s); return; }

    int hist[256] = {0};                                   // grayscale histogram
    for (int y = 0; y < H; ++y) {
        Uint8 *p = (Uint8*)s->pixels + (roi.y + y)*s->pitch + roi.x*4;  // pointer to row start
        for (int x = 0; x < W; ++x) {
    ...
    if (BLACK_THR < 20)  BLACK_THR = 20;                   // clamp very low values
    if (BLACK_THR > 235) BLACK_THR = 235;                  // clamp very high values

    for (int y = 0; y < H; ++y)
        for (int x = 0; x < W; ++x) {
            Uint8 Y = G[y*W + x];
            bin[y*W + x] = (Y < BLACK_THR) ? 0 : 255;      // 0 = black (ink), 255 = white
        }
...
static void horiz_proj(const Uint8 *bin, int w, int h, int *hp) {
    for (int y = 0; y < h; ++y) {
        ...
        for (int x = 0; x < w; ++x) s += (row[x] == 0);    // count black pixels
        hp[y] = s;                                         // horizontal projection
    }
}

static void find_runs_over(const int *arr, int n, int thr, int minw,
                           Box *out, int *nout) {
    int o = 0, i = 0;
    while (i < n) {
        while (i < n && arr[i] <= thr) i++;                // skip below threshold
        if (i >= n) break;
        int a = i;
        while (i < n && arr[i] > thr) i++;                 // run of values > thr
        int b = i - 1;
...
static Uint8 *crop_resize_28(const Uint8 *bin, int W, int H, Box bb) {
    ...
    if (!out) return NULL;
    for (int i = 0; i < OUT*OUT; ++i) out[i] = 255;        // start with white background

    int cw = bb.w, ch = bb.h;                              // bounding box size
    if (cw < 1 || ch < 1) return out;                      // degenerate: return blank tile
    ...
    for (int yy = 0; yy < th; ++yy) {
        ...
        if (syy < 0) syy = 0; else if (syy >= H) syy = H-1;
...
static void free_out_matrix(Uint8 ***Mat, int N, int M) {
    ...
    for (int i = 0; i < N; ++i) {
        if (!Mat[i]) continue;
        for (int j = 0; j < M; ++j) free(Mat[i][j]);       // free each 28x28 tile
...
static int extract_words(SDL_Surface *surf, SDL_Rect list, WordMatrix *WM) {
    ...
    Uint8 *bin = (Uint8*)malloc((size_t)W * (size_t)H);    // binary image of the list
    if (!bin) return -1;
    binarize_roi(surf, list, bin);                         // threshold list region

    int *hp = (int*)malloc(sizeof(int) * H);               // horizontal projection
    if (!hp) { free(bin); return -2; }
    horiz_proj(bin, W, H, hp);                             // count strokes per row

    Box lineRuns[1024];                                    // potential text lines
    int nL = 0;
    find_runs_over(hp, H, (int)(0.02 * W), 4, lineRuns, &nL); // rows with enough black pixels
    free(hp);

    if (nL <= 0) { free(bin); return 0; }                  // no lines detected

    WM->n_lines = nL;
    WM->n_words = (int*)calloc((size_t)nL, sizeof(int));   // words per line
    WM->n_chars = (int**)calloc((size_t)nL, sizeof(int*)); // chars per word per line
    WM->tiles   = (Uint8****)calloc((size_t)nL, sizeof(Uint8***)); // tiles per word per line
    if (!WM->n_words || !WM->n_chars || !WM->tiles) {
        free(WM->n_words); free(WM->n_chars); free(WM->tiles);
    ...
    for (int L = 0; L < nL; ++L) {
        ...
        if (hL < 4) continue;                              // skip too small lines

        int lineSize = W * hL;
        unsigned char *vis = (unsigned char*)calloc((size_t)lineSize, 1); // visited mask
        int *qx = (int*)malloc(sizeof(int) * lineSize);     // BFS queue x
        int *qy = (int*)malloc(sizeof(int) * lineSize);     // BFS queue y
        if (!vis || !qx || !qy) {
            free(vis); free(qx); free(qy);
            continue;
        }
        ...
        for (int yy = 0; yy < hL; ++yy) {
            for (int xx = 0; xx < W; ++xx) {
                ...
                if (bin[(y0+yy)*W + xx] == 0 && !vis[idx]) { // black pixel not yet visited
                    if (nC >= MAX_CHARS_PER_LINE) break;    // avoid overflow
                    int front = 0, back = 0;
                    vis[idx] = 1;                           // mark first pixel as visited
                    qx[back] = xx; qy[back] = yy; back++;   // enqueue
                    ...
                    while (front < back) {                  // BFS over connected black pixels
                        ...
                        for (int k = 0; k < 4; ++k) {
                            ...
                            if (nx < 0 || nx >= W || ny < 0 || ny >= hL) continue;
                            int nidx = ny*W + nx;
                            if (!vis[nidx] && bin[(y0+ny)*W + nx] == 0) {
                                vis[nidx] = 1;
                                qx[back] = nx; qy[back] = ny; back++;  // add neighbour to queue
                                if (nx < minx) minx = nx;
                                if (nx > maxx) maxx = nx;
                                if (ny < miny_rel) miny_rel = ny;
                                if (ny > maxy_rel) maxy_rel = ny;
                            }
                        }
                    }
                    ...
                    int area = bb_w * bb_h;
                    if (area < 15 || bb_h < hL * 0.3f) continue; // reject tiny/noise components
                    ...
                    if (topRel < 0) topRel = 0;
                    if (botRel >= hL) botRel = hL - 1;
            ...
            if (nC >= MAX_CHARS_PER_LINE) break;                  // safety break
        }

        free(vis); free(qx); free(qy);
        if (nC <= 0) continue;                                    // no characters on this line

        qsort(charBoxes, (size_t)nC, sizeof(Box), cmp_box_x);     // sort characters left→right
        ...
        if (gaps) {
            ...
            qsort(gaps, (size_t)nGaps, sizeof(int), cmp_int);     // sort gaps to estimate spacing
        }
        ...
        if (!gaps || nGaps == 0) {
            ...
            thrGap = (median <= 0) ? 4 : (median * 2 + 2);        // gaps much larger ⇒ new word
        }
        ...
        for (int i = 0; i < nGaps; ++i) {
            ...
            if (gap < 0) gap = 0;
            if (gap > thrGap) nW++;                               // large gap → new word
        }

        WM->n_words[L] = nW;                                      // store number of words
        WM->n_chars[L] = (int*)calloc((size_t)nW, sizeof(int));   // char counts per word
        WM->tiles[L]   = (Uint8***)calloc((size_t)nW, sizeof(Uint8**)); // tiles per word
        if (!WM->n_chars[L] || !WM->tiles[L]) {
            free(WM->n_chars[L]); free(WM->tiles[L]);
            WM->n_chars[L] = NULL; WM->tiles[L] = NULL; WM->n_words[L] = 0;
            if (gaps) free(gaps);
            continue;
        }
        ...
        for (int i = 0; i < nGaps; ++i) {
            ...
            if (gap > thrGap) {
                ...
                wIdx++; count = 1;                                // start new word
            } else count++;
        }
        WM->n_chars[L][wIdx] = count;                             // last word count

        for (int w = 0; w < nW; ++w)
            WM->tiles[L][w] = (Uint8**)calloc((size_t)WM->n_chars[L][w], sizeof(Uint8*)); // allocate char slots

        wIdx = 0;
        int cInWord = 0;
        for (int i = 0; i < nC; ++i) {
            Box bb = charBoxes[i];
            Uint8 *tile = crop_resize_28(bin, W, H, bb);          // crop + resize character to 28x28
            ...
            if (i < nGaps) {
                ...
                int gap = charBoxes[i+1].x - leftEnd;
                if (gap < 0) gap = 0;
                if (gap > thrGap) { wIdx++; cInWord = 0; }        // start next word
            }
        }
        if (gaps) free(gaps);
    }
...
static int accept_or_candidates(const int *idx, const float *prob, int k,
    ...
    int vert  = (a==8 || a==10 || a==11 || a==19 || a==5); // letters with vertical strokes: I,K,L,T,F
    int round = (a==14|| a==3  || a==16 || a==2);          // round letters: O,D,Q,C
    int bowl  = (a==15|| a==1  || a==17);                  // bowl-type: P,B,R
    float margin = (vert || round || bowl) ? HARD_MARGIN : ACCEPT_MARGIN; // stricter margin for ambiguous shapes

    if (p1 >= ACCEPT_P1_THR && (p1 - p2) >= margin) {
        *accepted = a; *ncand = 0; return 1;               // one clean winner, no candidate list
    }

    float s = 0.f;
    for (int i = 0; i < k; ++i) s += prob[i];              // sum probabilities
...
static int ocr_tile_topk(Network *net, const Uint8 *buf784,
                         int k, int *idx, float *logp, float *prob) {
    ...
    if (mean < 0.5f)                                       // if mostly dark → probably inverted
        for (int i = 0; i < 784; ++i) x[i] = 1.f - x[i];   // invert so background is bright

    return smart_predict_k(net, x, k, idx, logp, prob);    // CNN forward + top-k
}

/* -------------------- Export grid + words to text file -------------------- */
static int save_export_file(const char *path,
                            int rows, int cols, char **grid,
                            char **words, int n_words) {
    FILE *f = fopen(path, "w");
    if (!f) { perror("fopen export"); return -1; }

    fprintf(f, "%d %d\n", rows, cols);                     // first line: grid size
    for (int i = 0; i < rows; ++i) {
        for (int j = 0; j < cols; ++j) {
            char c = grid[i][j];
            if (c < 'A' || c > 'Z') c = '?';               // sanitize invalid chars
            fprintf(f, "%c", c);
            if (j + 1 < cols) fprintf(f, " ");             // space-separated letters
        }
        fprintf(f, "\n");
    }
...
SDL_Surface *pipeline(SDL_Surface *surface, SDL_Renderer *render) {
    if (!surface || !render) return surface;               // safety guard

    SDL_Rect grid = {0,0,0,0}, list = {0,0,0,0};           // bounding boxes for grid & list
    ...
    int rc = extract_letters(surface,
                             grid.x, grid.y,
                             grid.x + grid.w - 1, grid.y + grid.h - 1,
                             &out_matrix, &out_N, &out_M); // segmentation of grid
    ...
    CellCand *cells   = (CellCand*)calloc((size_t)out_N*out_M, sizeof(CellCand)); // solver cells
    char    **grid_mat = (char**)malloc((size_t)out_N * sizeof(char*));           // char grid
    if (!cells || !grid_mat) {
        fprintf(stderr, "OOM cells/grid_mat\n");
        free_out_matrix(out_matrix, out_N, out_M);
        free(cells); free(grid_mat);
        return surface;
    }
    for (int i = 0; i < out_N; ++i) {
        grid_mat[i] = (char*)malloc((size_t)out_M * sizeof(char)); // one row of chars
        if (!grid_mat[i]) {
            for (int t = 0; t < i; ++t) free(grid_mat[t]);
            free(grid_mat); free_out_matrix(out_matrix, out_N, out_M); free(cells);
            return surface;
        }
    }
    ...
    for (int i = 0; i < out_N; ++i) {
        for (int j = 0; j < out_M; ++j) {
            ...
            for (int t = 0; t < k; ++t) {
                int cls = idx[t];
                if (cls >= 0 && cls < 26) prob[t] *= LETTER_PRIOR[cls]; // apply class prior
            }

            for (int a = 0; a < k-1; ++a)                  // re-sort by prob after prior
                for (int b = a+1; b < k; ++b)
                    if (prob[b] > prob[a]) {
                        float tf = prob[a]; prob[a] = prob[b]; prob[b] = tf;
                        float tl = logp[a]; logp[a] = logp[b]; logp[b] = tl;
                        int   ti = idx[a];  idx[a]  = idx[b];  idx[b]  = ti;
                    }

            float s = 0.f;
            for (int t = 0; t < k; ++t) s += prob[t];      // renormalize probabilities
            if (s < 1e-12f) s = 1e-12f;
            for (int t = 0; t < k; ++t) {
                prob[t] /= s;
    ...
    for (int i = 0; i < out_N; ++i) {
        for (int j = 0; j < out_M; ++j) printf("%c ", grid_mat[i][j]);
        printf("\n");
    }
    printf("========================================\n");
    ...
    char **words = (char**)calloc((size_t)words_cap, sizeof(char*)); // dynamic array of words
            ...
            for (int L = 0; L < WM.n_lines; ++L) {
                for (int Wd = 0; Wd < WM.n_words[L]; ++Wd) {
                    ...
                    if (words_cnt >= words_cap) {          // grow buffer if needed
                        ...
                        if (!tmp) { fprintf(stderr, "realloc words OOM\n"); break; }
                        words = tmp;
                    }

                    words[words_cnt] = (char*)malloc((size_t)nC + 1); // allocate word string
                    for (int C = 0; C < nC; ++C) {
                        Uint8 *buf = WM.tiles[L][Wd][C];   // 28x28 tile for this character
                        if (!buf) { words[words_cnt][C] = '?'; continue; }
                        int   idx[KTOP];
                        float logp[KTOP], prob[KTOP];
                        int kk = ocr_tile_topk(&net, buf, KTOP, idx, logp, prob);
                        (void)kk;                          // we only use the top1
                        words[words_cnt][C] = (char)('A' + idx[0]); // best guess for this letter
                    }
    ...
    double stepX = (out_M > 0) ? ((double)grid.w / (double)out_M) : 0.0; // width per cell
    double stepY = (out_N > 0) ? ((double)grid.h / (double)out_N) : 0.0; // height per cell
    double base  = (stepX < stepY) ? stepX : stepY;        // use smallest dimension as reference

    int outline_width  = (int)(0.90 * base);               // approximate visual width of word box
    if (outline_width < 1) outline_width = 1;
    int outline_stroke = 2;                                // thickness of outline

    SDL_SetRenderDrawColor(render, 255, 0, 0, 255);        // red outlines for found words
    ...
    for (int wi = 0; wi < words_cnt; ++wi) {
        ...
        for (int k = 0; k < L; ++k) {
            char c = Wstr[k];
            if (c < 'A' || c > 'Z') { has_non_letter = 1; break; } // skip weird words
        }

        int   out[4];
        float sc = 0.f;
        if (!has_non_letter)
            resolution_prob(cells, grid_mat, out_N, out_M, Wstr, out, &sc); // probabilistic matching
        else
            out[0] = -1;

        if (out[0] == -1) {                                // fallback to exact search if prob fails
            int out2[4];
            resolution(grid_mat, out_N, out_M, Wstr, out2); // exact matching without probabilities
            if (out2[0] == -1) {
            ...
            int x1 = grid.x + (int)lround(cx0);            // translate to image coordinates
            int y1 = grid.y + (int)lround(cy0);
            int x2 = grid.x + (int)lround(cx1);
            int y2 = grid.y + (int)lround(cy1);

            draw_outline(render, x1, y1, x2, y2, outline_width, outline_stroke); // draw path rectangle
            printf("Found exact (fallback): %s  (%d,%d)->(%d,%d)\n",
                   Wstr, out2[0], out2[1], out2[2], out2[3]);
            continue;
        }
        ...
        int x1 = grid.x + (int)lround(cx0);                // translate to global coordinates
        int y1 = grid.y + (int)lround(cy0);
        int x2 = grid.x + (int)lround(cx1);
        int y2 = grid.y + (int)lround(cy1);

        draw_outline(render, x1, y1, x2, y2, outline_width, outline_stroke); // highlight matched word

        float mean_log = sc / (float)L;                    // average log-score per letter
        printf("Found prob (matches/prefix first): %s  (%d,%d)->(%d,%d)  score=%.3f  mean=%.3f\n",
    ...
    if (words) {
        for (int i = 0; i < words_cnt; ++i) free(words[i]);
        free(words);
    }
    free_out_matrix(out_matrix, out_N, out_M);             // free grid tiles
    free(cells);                                           // free solver cells
    for (int i = 0; i < out_N; ++i) free(grid_mat[i]);     // free grid rows
    free(grid_mat);
...
int main(int argc, char **argv) {
    ...
    SDL_Surface *loaded = IMG_Load(argv[1]);               // load input image
    if (!loaded) {
    ...
    SDL_Surface *surface = SDL_ConvertSurfaceFormat(loaded, SDL_PIXELFORMAT_ARGB8888, 0); // force ARGB8888
    SDL_FreeSurface(loaded);
    if (!surface) {
        fprintf(stderr, "ConvertSurfaceFormat failed\n");
        IMG_Quit(); SDL_Quit();
        return 1;
    }
    SDL_SetSurfaceBlendMode(surface, SDL_BLENDMODE_NONE);  // no blending when drawing

    SDL_Renderer *render = SDL_CreateSoftwareRenderer(surface); // renderer drawing directly on surface
    if (!render) {
    ...
    (void)pipeline(surface, render);                       // run full OCR + solver + drawing

    if (IMG_SavePNG(surface, "result.png") != 0)           // save annotated image
        fprintf(stderr, "IMG_SavePNG failed: %s\n", IMG_GetError());
    else
        printf("Result saved: result.png\n");
