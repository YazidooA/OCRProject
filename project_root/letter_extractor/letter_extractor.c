...
static void thin_zhang_suen_28(unsigned char *mask)
{
    ...
    do {
        ...
        for (int i = 0; i < W * H; ++i)
            if (to_remove[i] && mask[i]) { mask[i] = 0; changed = 1; }
...
static void maybe_thin_letter(Uint8 *buf784)
{
    ...
    for (int i = 0; i < N; ++i) {
        Uint8 v = buf784[i];
        if (v < 128) dark_count++; else light_count++;
    }
    if (dark_count == 0 && light_count == 0) return;
    ...
    for (int i = 0; i < N; ++i) {
        ...
        comp_count++;
        if (comp_count >= N) break; // safety
    }

    if (comp_count == 0) {
        for (int i = 0; i < N; ++i) buf784[i] = original[i];
        return;
    }
    ...
    int black_cc = 0;
    for (int i = 0; i < N; ++i) {
        if (labels[i] == best_comp) { mask_cc[i] = 1; black_cc++; }
        else                         mask_cc[i] = 0;
    }
    if (black_cc == 0) {
        for (int i = 0; i < N; ++i) buf784[i] = original[i];
    ...
    for (int i = 0; i < N; ++i) zoom_mask[i] = mask_cc[i];
    ...
    if (S < S_MIN) {
        ...
        for (int i = 0; i < N; ++i) tmp_mask[i] = 0;
        ...
        for (int i = 0; i < N; ++i) dil_mask[i] = 0;

        for (int y = 0; y < H; ++y) {
            for (int x = 0; x < W; ++x) {
                int idx = y * W + x;
                if (!tmp_mask[idx]) continue;
        ...
        int nb_zoom_black = 0;
        for (int i = 0; i < N; ++i)
            if (dil_mask[i]) nb_zoom_black++;

        if (nb_zoom_black > 0)
            for (int i = 0; i < N; ++i) zoom_mask[i] = dil_mask[i];
    }
    ...
    if (count > 0) {
        ...
        if (shift_x != 0 || shift_y != 0) {
            ...
            for (int i = 0; i < N; ++i) rec_mask[i] = 0;
            ...
            if (rec_black > 0)
                for (int i = 0; i < N; ++i) zoom_mask[i] = rec_mask[i];
        }
    }
    ...
    // ---- DEBUG: save a few tiles after thinning (max 5) ----
    {
        ...
        if (debug_count < 5) {
            SDL_Surface *dbg = SDL_CreateRGBSurfaceWithFormat(0, W, H, 32,
...
int extract_letters(SDL_Surface *src,
                    ...
                    int *out_M)
{
    if (!src || !out_matrix || !out_N || !out_M) return -1;
    ...
    if (x1 < 0) x1 = 0;
    if (y1 < 0) y1 = 0;
    if (x2 >= s32->w) x2 = s32->w - 1;
    if (y2 >= s32->h) y2 = s32->h - 1;

    int rw = x2 - x1 + 1;
    int rh = y2 - y1 + 1;

    // Create an ROI surface
    SDL_Surface *roi = SDL_CreateRGBSurfaceWithFormat(0, rw, rh, 32,
                                                      SDL_PIXELFORMAT_ARGB8888);
    if (!roi) {
        SDL_FreeSurface(s32);
        return -4;
    ...
    Uint8 *G = (Uint8*)malloc((size_t)rw * (size_t)rh);
    if (!G) {
    ...
    for (int x = 0; x < rw; ++x) {
        int s = 0;
        for (int y = 0; y < rh; ++y) s += (G[y * rw + x] < BLACK_THR);
        px[x] = s;
    }
    for (int y = 0; y < rh; ++y) {
        int s = 0;
        for (int x = 0; x < rw; ++x) s += (G[y * rw + x] < BLACK_THR);
        py[y] = s;
    }

    // Smoothing window sizes
    int wx = rw / 60; if (wx < 5) wx = 5; if (!(wx & 1)) wx++;
    int wy = rh / 60; if (wy < 5) wy = 5; if (!(wy & 1)) wy++;
    int hx = wx / 2, hy = wy / 2;
    ...
    if (!sx || !sy) {
        free(px); free(py); free(sx); free(sy); free(G);
        SDL_UnlockSurface(roi);
        SDL_FreeSurface(roi);
        SDL_FreeSurface(s32);
        return -9;
    ...
    for (int i = 0; i < rw; ++i) {
        ...
        int b = i + hx; if (b >= rw) b = rw - 1;
        for (int t = a; t <= b; ++t) s += px[t];
        sx[i] = (int)(s / (b - a + 1));
    }

    for (int i = 0; i < rh; ++i) {
        long long s = 0;
        int a = i - hy; if (a < 0) a = 0;
        int b = i + hy; if (b >= rh) b = rh - 1;
        for (int t = a; t <= b; ++t) s += py[t];
        sy[i] = (int)(s / (b - a + 1));
    }
    ...
    int minLagX = rw / 40; if (minLagX < 6) minLagX = 6;
    int maxLagX = rw / 2;  if (maxLagX <= minLagX) maxLagX = minLagX + 1;

    int minLagY = rh / 40; if (minLagY < 6) minLagY = 6;
    int maxLagY = rh / 2;  if (maxLagY <= minLagY) maxLagY = minLagY + 1;
        ...
        for (int j = 0; j < M; ++j) {
            ...
            for (int y = yy1; y <= yy2 && !black; ++y) {
                ...
                for (int x = xx1; x <= xx2; ++x)
                    if (rowG[x] < BLACK_THR) { black = 1; break; }
            }
            if (!black) { Mat[i][j] = NULL; continue; }
            ...
            SDL_Surface *sq = SDL_CreateRGBSurfaceWithFormat(0, s, s, 32,
                                                             SDL_PIXELFORMAT_ARGB8888);
            if (!sq) { Mat[i][j] = NULL; continue; }
