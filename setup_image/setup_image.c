...
void initialize(SDL_Window **window, SDL_Renderer **renderer, SDL_Texture **texture, SDL_Surface **surface, char* file){
    ...
    *texture = SDL_CreateTexture(*renderer, SDL_PIXELFORMAT_RGBA8888, SDL_TEXTUREACCESS_TARGET, width, height);
    ...
    if (file != NULL) {
        ...
        if (!temp_surface) {
            errx(1, "Failed to load image '%s': %s", file, IMG_GetError());
        }
        ...
        *surface = SDL_ConvertSurfaceFormat(temp_surface, SDL_PIXELFORMAT_RGBA8888, 0);
        ...
        if (!temp_texture) errx(1, "SDL Texture creation from surface failed: %s", SDL_GetError());

        if (SDL_SetRenderTarget(*renderer, *texture)) {
            errx(1, "Failed to set render target: %s", SDL_GetError());
        }

        if (SDL_RenderCopy(*renderer, temp_texture, NULL, NULL)) {
            errx(1, "Failed to copy texture: %s", SDL_GetError());
        }
...
int main(int argc, char *argv[]) {
    ...
    initialize(&window, &renderer, &texture, &surface, file);
